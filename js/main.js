...
setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    // Show hero logo — reveal from center outward with soft edges
    heroLogo.style.opacity = '0';
    heroLogo.style.maskImage = 'linear-gradient(to right, transparent, black 40%, black 60%, transparent)';
    heroLogo.style.webkitMaskImage = 'linear-gradient(to right, transparent, black 40%, black 60%, transparent)';
    heroLogo.style.maskSize = '50% 100%';
    heroLogo.style.webkitMaskSize = '50% 100%';
    heroLogo.style.maskPosition = 'center';
    heroLogo.style.webkitMaskPosition = 'center';
    heroLogo.style.maskRepeat = 'no-repeat';
    heroLogo.style.webkitMaskRepeat = 'no-repeat';
    setTimeout(() => {
      heroLogo.style.transition = 'opacity 1.5s ease, mask-size 2s ease, -webkit-mask-size 2s ease';
      heroLogo.style.opacity = '1';
      heroLogo.style.maskSize = '300% 100%';
      heroLogo.style.webkitMaskSize = '300% 100%';
    }, 200);
    // Character blur-in animation for hero text
    function splitAndAnimate(el, baseDelay, stagger) {
      if (!el) return 0;
      ...
      parts.forEach((part, pi) => {
        ...
        for (let i = 0; i < part.length; i++) {
          ...
          if (ch === ' ') {
            ...
            s.className = 'char-blur-space';
            s.style.animationDelay = (baseDelay + charIndex * stagger) + 's';
            el.appendChild(s);
          } else {
            const s = document.createElement('span');
            s.className = 'char-blur';
            s.textContent = ch;
            s.style.animationDelay = (baseDelay + charIndex * stagger) + 's';
            el.appendChild(s);
          }
    ...
    // Animate hero words
    const hwOwnInner = document.getElementById('hero-changing-word');
    const hwTheEl = document.getElementById('hw-the');
    const hwMarketEl = document.getElementById('hw-market');
    splitAndAnimate(hwOwnInner, 0.2, 0.08);
    splitAndAnimate(hwTheEl, 0.5, 0.08);
    splitAndAnimate(hwMarketEl, 0.8, 0.08);

    // Animate tagline
    const tagHeading = document.querySelector('.hero-tagline-heading');
    ...
    initDoorSystem();
    // Hero intro door — play as timed image sequence (~30fps)
    const heroFrame = document.getElementById('hero-bg-frame');
    if (heroFrame) {
      const S1_TOTAL = 30;
      const S1_FPS = 30;
      const s1Srcs = [];
      ...
      // Wait for all frames to load, then play
      const waitLoad = setInterval(() => {
        if (s1Loaded >= S1_TOTAL) {
          clearInterval(waitLoad);
          let frame = 0;
          heroFrame.src = s1Srcs[0];
          const playInterval = setInterval(() => {
            frame++;
            if (frame >= S1_TOTAL) {
              clearInterval(playInterval);
              // Freeze on last frame, hand off to scroll sequence
              window.dispatchEvent(new CustomEvent('portal-show'));
              return;
            }
            heroFrame.src = s1Srcs[frame];
          }, 1000 / S1_FPS);
        }
      }, 50);
    }
...
/* ---- SCROLL-DRIVEN PORTAL ZOOM — handled by js/portal-sequence.js ---- */
function initDoorSystem() {
  // Portal canvas sequence is in portal-sequence.js
}
...
function initLogoAnimation() {
  ...
  function calcLogoLayout() {
    startTop = window.innerHeight * 0.43;
